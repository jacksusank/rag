...
def findSimilarVectors(user_tuple):
    ...
    # My computer's connection to PostgreSQL
    # connection = psycopg2.connect(
    #     host="localhost",
    #     port="5432",
    #     database="vector_db",
...
while (my_input != "quit"):
    ...
    vectorized_question = model.encode(my_input)
    ...
    embedded_input = vectorized_question
    ...
    fully_formatted_input = [embedding.tolist() for embedding in embedded_input]
    print(embedded_input[0])
    print(embedded_input[1])


    print("\n\nResults: ")
    llmResponse = (chatWithLLM(promptMaker(findSimilarVectors((fully_formatted_input, my_input)))))
    print(llmResponse)
    print("\n\n")
